...
public class WebServer {
    ...
    /** Bitmask for extracting individual bytes from the array. */
    private static final int BYTE_MASK = 0xFF;

    /*
     * Various bit shifts for RGBA-packed data. Red is the lowest byte, green is next, followed by
     * blue and alpha.
     */
    ...
    private static void wrapImageTransform(final RoutingContext routingContext) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                pixels[x][y] |= ((data[dataIndex++] & BYTE_MASK) << RED_SHIFT);
                pixels[x][y] |= ((data[dataIndex++] & BYTE_MASK) << GREEN_SHIFT);
                pixels[x][y] |= ((data[dataIndex++] & BYTE_MASK) << BLUE_SHIFT);
                pixels[x][y] |= ((data[dataIndex++] & BYTE_MASK) << ALPHA_SHIFT);
            }
        }
        ...
        try {
            switch (transformation) {
                ...
                case "mystery":
                    ...
                    break;
                case "blur":
                    transformedPixels = Transform.blur(pixels);
                    break;
                case "monochrome":
                    transformedPixels = Transform.monochrome(pixels);
                    break;
                case "increaseBrightness":
                    transformedPixels = Transform.increaseBrightness(pixels);
                    break;
                case "decreaseBrightness":
                    transformedPixels = Transform.decreaseBrightness(pixels);
                    break;
                default:
        ...
        for (int y = 0; y < transformedPixels[0].length; y++) {
            for (int x = 0; x < transformedPixels.length; x++) {
                transformedData[dataIndex++] = //
                        (byte) ((transformedPixels[x][y] >> RED_SHIFT) & BYTE_MASK);
                transformedData[dataIndex++] = //
                        (byte) ((transformedPixels[x][y] >> GREEN_SHIFT) & BYTE_MASK);
                transformedData[dataIndex++] = //
                        (byte) ((transformedPixels[x][y] >> BLUE_SHIFT) & BYTE_MASK);
                transformedData[dataIndex++] = //
                        (byte) ((transformedPixels[x][y] >> ALPHA_SHIFT) & BYTE_MASK);
            }
        }
